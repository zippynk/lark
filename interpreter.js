...
//The join_parsers function takes two parser functions and returns a function that attempts to parse an input string with one parser followed by the other. It does this by splliting the string in half at every index and checking whether each parser can parse its half of the string.
function join_parsers(left_parser,right_parser){
  ...
  return _.memoize(function(str_to_check){
    ...
    var left_attempt;
    var right_attempt;
    //Yes, we could have weird recursing but we won't since I'll never pass a function to its own left_parser or right_parser or something similar with more levels.
...
function make_rule(parser,postprocessor){
  return {parser:parser,postprocessor:postprocessor};
}
...
function with_parser(parser){
  return rule_from_func(
    function(str_to_parse){
      ...
      console.log(given_parser_attempt);
...
function rule_from_func(func_to_use){
  ...
  return {parse:function(str_to_check){
